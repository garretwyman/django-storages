    ...
    def _get_file(self):
        ...
        if "r" in self._mode or "a" in self._mode:
            download_stream = self._storage.client.download_blob(
                self._path, timeout=self._storage.timeout, headers=_make_headers()
            )
...
class AzureStorage(BaseStorage):
    ...
    def exists(self, name):
        ...
        return blob_client.exists(headers=_make_headers())

    def delete(self, name):
        try:
            self.client.delete_blob(
                self._get_valid_path(name), timeout=self.timeout, headers=_make_headers()
            )
    ...
    def list_all(self, path=""):
        ...
        return [
            ...
            for blob in self.client.list_blobs(
                name_starts_with=path, timeout=self.timeout, headers=_make_headers()
            )
        ]
